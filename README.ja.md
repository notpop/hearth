...
タスクは Nix flake apps として定義。`nix develop`(あるいは `direnv allow` 済み)のシェル内では各タスクが**シェル関数**として定義され、`nix run .#` 無しで直接呼べます:

```sh
nix develop                  # dev shell に入る(direnv ユーザは自動)
...
非対話用(CI、シェルスクリプト等)は flake apps として呼ぶ。`#` は flake で attribute を指定する Nix の固定構文:
...
別の Nix flake から Hearth を build input として取り込めます:

```nix
inputs.hearth.url = "github:notpop/hearth";
# devShell に: hearth.packages.${system}.default
```
