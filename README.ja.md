...
タスクは Nix flake apps として定義。`nix run .#<name>` で実行:

```sh
nix run .#build              # ./bin/hearth
nix run .#test               # テスト一式(~10 秒)
nix run .#test-race
nix run .#cover              # パッケージ別カバレッジ
nix run .#vet
nix run .#lint               # vet + staticcheck
nix run .#proto              # gRPC スタブ再生成
...
ツール一式が `$PATH` に揃った対話シェルが必要なら `nix develop`(Go 1.26、gopls、golangci-lint、sqlite、protoc 等)。
